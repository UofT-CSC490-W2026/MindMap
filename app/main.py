...
def build_parser() -> argparse.ArgumentParser:
	parser = argparse.ArgumentParser(
		description="MindMap backend runner (Modal worker orchestrator)."
	)
...
def main() -> None:
	...
	if args.command == "ingest":
		result = ingest_from_arxiv.remote(query=args.query, max_results=args.max_results)
	...
	if args.command == "embed":
		result = run_embedding_batch.remote(limit=args.limit, model_name=args.model_name)
		_print_result(result)
		return

	if args.command == "citation-embed":
		result = run_citation_aware_embedding_batch.remote(
			limit=args.limit,
			alpha=args.alpha,
	...
	if args.command == "related":
		result = get_related_papers.remote(paper_id=args.paper_id, k=args.k)
		_print_result(result)
		return

	if args.command == "pipeline":
		print("Step 1/3: ingest")
		ingest_from_arxiv.remote(query=args.query, max_results=args.max_results)

		print("Step 2/3: transform")
		transform_to_silver.remote()

		print("Step 3/3: embed")
		result = run_embedding_batch.remote(limit=args.embed_limit, model_name=args.model_name)
		_print_result(result)
		return
